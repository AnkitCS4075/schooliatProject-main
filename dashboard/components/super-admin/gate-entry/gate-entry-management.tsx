...
import { useGateEntries, useGateEntryStats, useCreateGateEntry, useUpdateGateEntry, useDeleteGateEntry, type GateEntry } from "@/lib/hooks/use-gate-entry";
...
import { Plus, LogOut, Trash2, Search, Users, DoorOpen, ClipboardList, PhoneCall, Building2 } from "lucide-react";
...
export function SuperAdminGateEntryManagement() {
  const [page, setPage] = useState(1);
  const [categoryFilter, setCategoryFilter] = useState<string>("");
  const [schoolFilter, setSchoolFilter] = useState<string>("");
  const [search, setSearch] = useState("");
  const [addDialogOpen, setAddDialogOpen] = useState(false);
  const [form, setForm] = useState({ schoolId: "", category: "", name: "", phone: "", reason: "", classInterestedIn: "", personToMeet: "" });
  ...
  const list = (data ?? {}) as GateEntryListResponse;
  const entries = list.data ?? [];
  const totalPages = list.totalPages ?? 1;
  const stats = (statsData as { data?: GateEntryStats })?.data;

  const handleCreate = async () => {
    if (!form.schoolId || !form.category || !form.name || !form.phone) {
      toast({ title: "Validation Error", description: "School, category, name, and phone are required", variant: "destructive" });
      return;
    }
    try {
      ...
      setForm({ schoolId: "", category: "", name: "", phone: "", reason: "", classInterestedIn: "", personToMeet: "" });
    } catch {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Gate Entry — All Schools</h1>
          <p className="text-muted-foreground text-sm mt-1">Visitor &amp; admission-enquiry registers across every school. Filter by school or record a new entry on behalf of a school.</p>
        </div>
      ...
      <div className="border rounded-lg">
        <Table>
          <TableHeader><TableRow><TableHead>#</TableHead><TableHead>School</TableHead><TableHead>Date &amp; Time</TableHead><TableHead>Category</TableHead><TableHead>Name</TableHead><TableHead>Phone</TableHead><TableHead>Reason</TableHead><TableHead>Class</TableHead><TableHead>Person to Meet</TableHead><TableHead>Out Time</TableHead><TableHead>CRM Lead</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {isLoading ? <TableRow><TableCell colSpan={12} className="text-center py-8">Loading...</TableCell></TableRow>
            : entries.length === 0 ? <TableRow><TableCell colSpan={12} className="text-center py-8 text-muted-foreground">No entries found</TableCell></TableRow>
            : entries.map((e) => (
              <TableRow key={e.id}>
                <TableCell className="font-mono">{e.serialNo}</TableCell>
                <TableCell><Badge variant="outline" className="bg-slate-100 text-slate-800"><Building2 className="h-3 w-3 mr-1" />{e.school?.code ?? "—"}</Badge></TableCell>
                ...
                <TableCell>
                  {e.linkedLead ? (
                    <a href={`/super-admin/leads?lead=${e.linkedLead.id}`}><Badge className="bg-green-100 text-green-800 hover:bg-green-200 cursor-pointer">Lead Created</Badge></a>
                  ) : (
